...
export default function PlaylistDetail() {
  ...
  const { data: playlists = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Playlist.filter({ id: playlistId }),
  });
  ...
  const tracks = playlist?.track_ids?.map(id => allTracks.find(t => t.id === id)).filter(Boolean) || [];
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      {tracks.length === 0 && (
        <p className="text-center text-muted-foreground py-10">This playlist is empty. Add some tracks!</p>
      )}
    </div>
  );
}