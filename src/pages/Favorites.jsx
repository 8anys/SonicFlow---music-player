...
import { getFavoriteTracks, removeFavoriteTrack } from '@/api/favorites';
...
export default function Favorites() {
  ...
  const removeMutation = useMutation({
    mutationFn: (track) => removeFavoriteTrack(track.id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['favorite-tracks'] }),
  });
          ...
          <div className="space-y-0.5">
            {tracks.map((track, index) => (
              <TrackRow
                ...
                tracks={tracks}
                isLiked
                onLike={(selectedTrack) => removeMutation.mutate(selectedTrack)}
              />
            ))}
          </div>
        </>
